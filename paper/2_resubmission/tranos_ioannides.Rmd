...
major areas, that is, cities, from one another, versus dispersion within major
...
issue, as it is clear that not all economic activities might continue in an
unencumbered manner; see Dingel and Neiman [@dingel2020many].
\color{black}
...
kable(zipf.table, format = "latex",
      caption = "Pareto exponent\\label{zipf}",
      booktabs = T,
      digits = 2) %>%
...
stargazer(net.2sls, bb.2sls, fixed.2sls, mobile.2sls, type = "latex",
          ...
          notes = c("Robust std. errors in parenthesis",
                    ...
                    "First stage regressions can be found in S2"))
```
...
more active engage with the internet, and, therefore, experience a larger effect
...
does not make up for physical distance; individuals nearer one another
communicate more.  
\color{black}
...
\color{blue}
The coefficients derived from OLS (Table \ref{ols.uk}) and 2SLS (Table \ref{2sls.uk}) are always positive and significant, something which advocates towards the effect of broadband speed in enhancing agglomeration forces.
